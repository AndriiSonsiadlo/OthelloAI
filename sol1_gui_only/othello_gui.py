...
class OthelloGUI:
    def __init__(self):
        ...
        self._menu_bar = tk.Menu(self._root_window)
        self._game_menu = tk.Menu(self._menu_bar, tearoff=0)
        ...
        self._black_score.get_score_label().grid(row=0, column=0, sticky=tk.S)
        self._white_score.get_score_label().grid(row=0, column=1, sticky=tk.S)
        self._board.get_board().grid(row=1, column=0, columnspan=2, padx=50, pady=10, sticky=tk.N + tk.E + tk.S + tk.W)
    ...
    def _on_board_clicked(self, event: tk.Event) -> None:
    ...
    def _on_board_resized(self, event: tk.Event) -> None:
